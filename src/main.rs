...
enum UpdateCheck {
    UpToDate,
    Updated,   // remote digest differs from local (for the same platform)
...
fn detect_update_for_image(image: &str) -> Result<UpdateCheck> {
    ...
    let local_digests = get_local_digests(image)?;
    ...
    // 3) Get the single remote digest for the matching platform
    let remote_digest = match get_remote_platform_digest(image, &local_platform.unwrap()) {
        Ok(Some(d)) => d,
        Ok(None) => {
    ...
    // 4) Compare single per-platform manifest digest against local RepoDigests
    let local_set: BTreeSet<_> = local_digests.into_iter().collect();
    if local_set.contains(&remote_digest) {
...
fn get_remote_platform_digest(
    image: &str,
    platform: &(String, String, Option<String>),
) -> Result<Option<String>> {
    // Prefer verbose to get per-platform entries
    ...
    if let Ok(o) = verbose {
        if o.status.success() {
            ...
            // Multi-arch: manifests[] with platform info
            if let Some(d) = pick_digest_from_verbose_manifest(&val, platform) {
                return Ok(Some(d));
            }

            // Single-manifest case: some engines include Descriptor.digest
            if let Some(d) = pick_descriptor_digest(&val) {
                return Ok(Some(d));
            }
    ...
    // Fallback: non-verbose (may work for single-arch manifests)
    let output = Command::new("docker")
        .arg("manifest")
        .arg("inspect")
        .arg(image)
    ...
    // Single-manifest: Descriptor.digest
    if let Some(desc) = val.get("Descriptor").and_then(|d| d.as_object()) {
        if let Some(d) = desc.get("digest").and_then(|v| v.as_str()) {
    ...
    // Multi-arch without platform info (unlikely), cannot safely pick
    Ok(None)
}
...
fn get_local_digests(image: &str) -> Result<Vec<String>> {
    let output = Command::new("docker")
        .arg("image")
        .arg("inspect")
        .arg("--format")
    ...
    let val: json::Value = json::from_str(&text).unwrap_or(json::Value::Null);
    Ok(parse_repo_digests(&val))
}

// --------- Pure parsing helpers for testability ---------
fn parse_repo_digests(val: &json::Value) -> Vec<String> {
    let mut digests = Vec::new();
    match val {
        json::Value::Array(arr) => {
            for v in arr {
                if let Some(s) = v.as_str() {
                    let digest = s.split('@').last().unwrap_or("").to_string();
                    if !digest.is_empty() {
                        digests.push(digest);
                    }
                }
    ...
    let set: BTreeSet<_> = digests.into_iter().collect();
...
fn pick_digest_from_verbose_manifest(
    val: &json::Value,
    platform: &(String, String, Option<String>),
) -> Option<String> {
...
fn pick_descriptor_digest(val: &json::Value) -> Option<String> {
    val.get("Descriptor")
...
"#;
    ...
    #[test]
    fn test_parse_repo_digests_array_and_dedup() {
        let val = json::json!(["nginx@sha256:aaa", "repo@sha256:aaa", "repo@sha256:bbb"]);
        let mut digs = parse_repo_digests(&val);
        digs.sort();
        assert_eq!(digs, vec!["sha256:aaa", "sha256:bbb"]);
    }

    #[test]
    fn test_parse_repo_digests_string() {
        let val = json::json!("nginx@sha256:deadbeef");
        let digs = parse_repo_digests(&val);
        assert_eq!(digs, vec!["sha256:deadbeef"]);
    }

    #[test]
    fn test_parse_repo_digests_null_empty() {
    ...
    #[test]
    fn test_pick_digest_from_verbose_manifest_exact_match() {
        ...
        let d = pick_digest_from_verbose_manifest(&manifest, &plat).unwrap();
        assert_eq!(d, "sha256:222");
    }

    #[test]
    fn test_pick_digest_from_verbose_manifest_variant_tolerant() {
        let manifest = json::json!({
            "manifests": [
                {
                    "digest": "sha256:333",
        ...
        let plat = ("linux".into(), "arm64".into(), None);
        let d = pick_digest_from_verbose_manifest(&manifest, &plat).unwrap();
        assert_eq!(d, "sha256:333");
    }

    #[test]
    fn test_pick_descriptor_digest() {
        let val = json::json!({"Descriptor": {"digest": "sha256:ffff"}});
        let d = pick_descriptor_digest(&val).unwrap();
        assert_eq!(d, "sha256:ffff");
    }
}
